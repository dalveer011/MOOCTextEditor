...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		boolean lastWordWasAVowel = false;
		if(word == "Segue") {
		...
		for(int i=0;i<word.length();i++) {
		  char letter = word.charAt(i);
			if(isCharVowel(letter)) {
				lastWordWasAVowel = true;
				continue;
			} else {
				if(lastWordWasAVowel)
					numberOfSyllables++;
				lastWordWasAVowel = false;
		...
		if(word.charAt(word.length()-1) == 'e' && numberOfSyllables > 1 && !isCharVowel(word.charAt(word.length()-2))  ) {
			numberOfSyllables--;
		}
	...
	public boolean isCharVowel(char testChar) {
		return (testChar == 'a' || testChar == 'e' || testChar == 'i' || testChar == 'o' || testChar == 'u' || testChar == 'y'
				||testChar == 'A' || testChar == 'E' || testChar == 'I' || testChar == 'O' || testChar == 'U' || testChar == 'Y');
	}
